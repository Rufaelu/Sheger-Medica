...
class RemediesController extends Controller
{
    ...
    public function all()
    {
        $remedies = Remedies::with(['herb', 'user'])->get();

        return view('remedies.all', ['remedies' => $remedies]);
    }

    // Show the form for creating a new remedy
    public function create()
    {
        $herbs = Herbs::all(); // Get all herbs for selection
...
// Attach the herbs to the remedy (many-to-many relationship)
    ...
    {
        $remedy = Remedies::with(['herb', 'user', 'reviews'])->findOrFail($id);
