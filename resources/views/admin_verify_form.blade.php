...
<body>
    ...
    <div id="applicationContainer" class="application">
        ...
        <div class="field">
            ...
            <div class="field-value">{{ $user->specialties }}</div>
        </div>
        <div class="field">
            <div style="width: 100%; height: auto; border: 1px solid #ccc; overflow: auto;">

                <iframe     src="{{ asset($user->certificate) }}" style="display:none;">
                </iframe>

            </div>
        </div>
        <div class="button-group" style="display: flex; justify-content:center">
            <form action={{route('approve',['id'=>$user->user_id])}} method="post">
                @csrf
                <button id="approveBtn">Approve</button>
            </form>
            <form action={{route('reject',['id'=>$user->user_id])}} method="post">
                @csrf
